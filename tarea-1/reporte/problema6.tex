...
\begin{tikzpicture}[node distance=1.0cm and 1.3cm]
    \node[wide] (t1) {T1: pp = (p1+p2+p3)/3};
    \node[wide, below=1.0cm of t1] (t2) {T2: pa = (a1+a2+a3+a4)/4};
    \node[wide, right=1.0cm of $(t1)!0.5!(t2)$] (t3) {T3: cf = 0.5*pp + 0.5*pa};
    \node[wide, right=1.0cm of t3] (t4) {T4: aprobado = (p1>=6) y ... y (cf>=6)};
    \node[wide, right=1.0cm of t4] (t5) {T5: imprimir nombre};
    \draw[dep] (t1) -- (t3);
    \draw[dep] (t2) -- (t3);
    \draw[dep] (t3) -- (t4);
    \draw[dep] (t4) -- (t5);
